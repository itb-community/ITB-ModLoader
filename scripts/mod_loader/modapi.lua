...
function modApi:deleteModdedMaps()
	for i, file in pairs(os.listfiles("maps")) do
		local mapname = string.gsub(file, "\.map", "")
		if
			modApi:stringEndsWith(file, ".map") and
			not list_contains(self.defaultMaps, mapname)
		then
			os.remove("maps/"..file)
		end
	end
...
function modApi:addMap(mapfile)
	local idx = (string.find(mapfile, "/[^/]*$") or 0) + 1
	local mapname = string.sub(mapfile, idx)
	self:copyfile(mapfile, "maps/"..mapname)
end
